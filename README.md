...
React Nativeの部品（`<View>` や `<Text>`）には、Web（HTMLなどを使うReactなど）よりも細かい設定（プロパティ）が無数にあります。TypeScriptを使うと、エディタが「ここにはこの型を入れてね」とガイドしてくれるため、辞書を引かずに開発できます。エディタはTypeScriptの開発元のマイクロソフト社のVSCや、それの派生なAIエディタであるCursorが主流です。TypeScriptは他の言語と違い構造がシンプルなので、開発速度などが向上します。
